...
`VaultStore` works in the destination directory. It creates an empty persistent `.<filename>.lock` sidecar (for a file name too long for that, the name is shortened like the migration copy's, see [FORMAT.md](FORMAT.md#compatibility-and-migrations)) and acquires an exclusive OS file lock to coordinate cooperating writers. The sidecar contains no vault state or secrets and must not be deleted while writers might use it. All actual vault data stays in one encrypted file.
...
The entry list's **Zuletzt verwendet** (*Recently used*) filter is session-only. It is fed by opening an entry in the editor, copying one of its values (including TOTP codes) and opening its link, and holds at most the ten most recent entry IDs of the open vault, never titles or values. The list exists only in the application's memory: it is never written to the settings file, the vault, backups, exports or logs, it is replaced by an empty list on every lock (manual, inactivity, window or OS event), and a use in another vault starts a new list, so entries of one vault are never offered for another. It does not survive a restart. Favorites, by contrast, are part of the vault document as the entry field `pinned` and are encrypted and backed up with it.
...
not an entropy estimate or a breached-password database; this local check makes no
breach lookup (see [Breach check](#breach-check) for the optional one the user starts).
...
notes, entry notes, customer and project notes, history and custom field values are never read, whatever their
hidden option says. Entry titles, tags, customer and project names, ports, protocols, key types and expiry dates are
metadata and do appear; the customer overview also shows the plain customer contact details (contact person,
e-mail, phone, website) and project descriptions, which are stored as metadata for exactly that purpose. The visible values that are read become immutable JVM strings, like the text shown in the detail view.
Trashed entries are left out. Tests close every hidden secret and all notes, including customer and project notes,
of a sample vault with marked values for every entry type before building each report, so a report that read one would fail, and check that none of
...
The **expiry export** (ICS or CSV) contains, per active entry with an expiry date, only the date, title, type,
customer and project names and the domain name of a visible domain field; never any other field value. ICS text
values are escaped (backslash, semicolon, comma, line breaks; other control characters become spaces) and lines
are folded at 75 octets without splitting a character, so a title cannot inject calendar properties. CSV cells
are always quoted, and cells starting with `=`, `+`, `-`, `@`, tab or carriage return get a leading `'` against
formula injection when opened in a spreadsheet. Both files are created through the same owner-only path. Once
imported into a calendar or a shared spreadsheet, the dates and titles are outside Keyrook's protection and may be
synchronized to other devices or services.
...
The second and last feature that makes Keyrook contact the network itself is the optional comparison with the [Pwned Passwords](https://haveibeenpwned.com/Passwords) range API of Have I Been Pwned. It runs only when the user clicks **Passwörter mit bekannten Datenlecks abgleichen …** (**Check passwords against known breaches …**) in the warning list and then confirms a question. It never runs automatically, at start, after unlocking or on a schedule; there is no setting for it and consent is never remembered, so every run asks again. The question starts with the focus on **Abbrechen** (**Cancel**), Enter alone does not confirm and Escape cancels. It names the number of passwords and requests, the service, that only hash prefixes are sent, that the service sees the IP address and that nothing is stored.

Before the question, the non-empty passwords of the active entries are hashed locally: the password fields, SSH key passphrases and custom fields named `password`, `passwort` or `passphrase` of the current data, not the history, notes or trash. Hashing runs on the erasable copy that `VaultSession.read` provides, without converting a secret to a string: the UTF-8 bytes from `Secret.useUtf8` go to the JDK's SHA-1 `MessageDigest`, and the encoded bytes, the digest and the hexadecimal character arrays are erased after use. Until the run ends, only the 35-character suffixes remain in character arrays that are erased when the run ends, the question is declined or the vault is locked. SHA-1 is required by the service; it is used for the lookup only, never for vault encryption.

Exactly what is sent, and to whom: for each distinct prefix, once per run, one HTTPS `GET https://api.pwnedpasswords.com/range/<first 5 hexadecimal characters of the SHA-1 hash>` with only the headers `Add-Padding: true` and `User-Agent: Keyrook/<version>` besides those HTTP itself requires. Identical prefixes are requested once, so the number of requests is at most the number of passwords. There are no query parameters, cookies, credentials, installation or user identifiers; passwords, full hashes, titles, user names, addresses and other vault data are never sent. At most two requests run at the same time. The client is the same kind as for the update check: `java.net.http.HttpClient` with the system proxy settings, no cookie store, no authenticator and no redirects, a 15-second connect and response timeout, at most 30 seconds per request and 10 minutes for the whole run. The run can be canceled at any time; running requests are then interrupted.

The response is untrusted. At most 512 KiB and 8192 lines are read per request; each line must be a 35-character hexadecimal suffix, a colon and a decimal count of at most 12 digits, separated by LF or CRLF. Padding records with count 0 are accepted and ignored. The suffixes are compared locally. Any status other than 200, transport error, invalid or oversized response, timeout or cancellation ends the whole run with a generic message and discards every result of that run; there is no partial result.

Results are the IDs of affected entries with their breach counts. They exist in memory only: they are never written to the vault, backups, exports, `settings.json` or any other file, and never logged. They appear as the warning reason *Passwort in bekannten Datenlecks* (*password in known breaches*) and are discarded on lock and when the window closes. Editing or trashing an entry removes its finding until the next run.

Threat notes:

- The service (operated by Have I Been Pwned behind Cloudflare) and any proxy in between see the requests, your IP address, the time and the number of requests, as with any HTTPS connection; their own privacy terms apply.
- k-anonymity hides which of the hundreds of hashes sharing a prefix is meant, not the prefixes themselves. The set of prefixes stays the same as long as the passwords do, so an observer of several runs can recognize the same vault across runs, networks and addresses, and the number of requests hints at the number of distinct passwords. Padding hides the size of the answers, not the requests.
- Someone who suspects a specific password can check whether its prefix was requested. That is weak evidence for a strong random password and stronger evidence for a guessable one.
- Counts come from the service. A manipulated service or a trusted-certificate interception can hide breaches or invent them; findings are hints and never change the vault. Absence from the corpus does not mean a password is strong or unknown to attackers.
...
The tests cover RFC 9106 Argon2id, NIST AES-256-GCM vectors, RFC 4226 HOTP and RFC 6238 TOTP vectors, a frozen format fixture, all eight entry variants, optional key files, tampering, limits, version rejection, absence of sentinel plaintext, failed writes, conflicts, permissions, credential changes and explicit buffer lifecycle. These tests are evidence for specific behavior, not a cryptographic audit or proof of crash safety. The production schema migration from schema 1 to 2 is covered with a realistic schema 1 document, the frozen schema 1 fixture, rejection of newer schemas and of schema 2 fields in a schema 1 document, and the copy of the old file kept by the first save; the pipeline itself is also covered with a synthetic older schema.

## Format migrations

Migrations run only after successful authentication, on the decrypted JSON tree, and only for an older schema with a complete registered chain; newer, unknown or unbridged versions are rejected. Steps must keep the vault ID and revision, and the result is re-encoded and checked by the same JSON guard, decoder and schema validation as a current document. The migration path creates additional transient plaintext: the JSON tree holds immutable strings that cannot be erased, and the intermediate re-encoded bytes are wiped. Opening never rewrites the file; the next ordinary save writes the current format atomically. Before that first save the session copies the unchanged older ciphertext, after checking it against the authenticated file, next to the vault as `<vault file>.schema-v<old>-r<revision>.keyrook.bak` (at most 255 bytes; long vault names are shortened, see [FORMAT.md](FORMAT.md#compatibility-and-migrations)) with owner-only permissions, whether or not a backup folder is configured; if it cannot be written, nothing is saved; the copy is still encrypted with the same credentials and is never rotated or deleted by Keyrook, so the user removes it once the upgrade is confirmed. A configured automatic backup of the older ciphertext follows as for any save. Current-version files use the unchanged decoding path apart from a streaming read of `schemaVersion`.

The schema 1 to 2 step moves the favorite tag into the `pinned` field and adds the empty template list; it touches no secret value. Customer and project notes added by schema 2 are secrets like entry notes (see above). Templates are layout only: type, field names, visible or hidden, field kind, tags and an optional customer and project. They never contain a field value, notes, port or entry reference, so saving an entry as a template copies no secret, and templates can appear unmasked in the interface. A schema 2 vault opened by an older release is rejected as an unsupported version without being modified.
