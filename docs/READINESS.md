...
| Desktop records | Eight entry types, customer/project editing, search, filters, sorting, trash, history, TOTP-secret editing and generators | TOTP values are stored, not used to generate one-time codes. German and English interface, selectable in the settings; standard system dialog buttons follow the Java runtime locale. |
