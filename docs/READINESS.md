...
| Desktop records | Eight entry types, customer/project editing, search, filters, sorting, trash, history, TOTP-secret editing with RFC 6238 code display/copy (RFC 4226/6238 test vectors) and generators | TOTP codes follow the local system clock; HOTP (counter-based) secrets and `otpauth` parameters beyond the documented set are refused. German and English interface, selectable in the settings; standard system dialog buttons follow the Java runtime locale. |
