...
The encoded document must also fit into the 64 MiB file (`MAX_FILE_BYTES`, header and tag included). The counts above allow far larger documents (10,000 entries with 100 history items each would not fit), so the file size is the binding limit: a save or export whose document would exceed it fails with a specific, content-free "vault too large" error (`VaultTooLargeException`) and leaves the file unchanged. Imports whose result alone could not be saved are refused the same way.

Before deserialization a guard rejects nesting beyond 32 levels, individual JSON strings above 1,572,865 encoded bytes (262,144 UTF-16 code units, each at most a six-byte `\uXXXX` escape, and the closing quote), containers above 10,000 members, malformed UTF-8, duplicate object keys (including equivalent escaped names), and more structural commas/colons than a third of the accepted document size (about 22.4 million for 64 MiB). That bound follows from the model: in the compact encoding every object member (`"key":value,`) spends at least seven bytes on its two separators, because field names have at least two characters and an empty custom label is followed by a whole field object, and every further list element spends at least three bytes on its comma (`"",` for an empty tag). A valid vault within the file size therefore never reaches it, while a hostile authenticated document still needs three bytes for every node the parser would create. The JSON decoder rejects unknown fields, types, malformed syntax and trailing documents. Schema validation follows authentication and parsing. Errors never include parser excerpts of decrypted data.
...
- lower, with a contiguous chain of registered `SchemaMigration(from, to)` steps: the decrypted JSON tree is transformed step by step in ascending order, then re-encoded and passed through the same guard, decoder and schema validation as a stored current document. The re-encoded document may exceed the file size by up to 4 MiB, because migration adds default fields (schema 1 to 2 adds at most about 1.2 MB for 10,000 customers, projects and entries); an older file close to the size limit therefore still opens, and saving it reports the too-large error until records are removed;
...
A migrated document is held only in memory; opening never writes the file. The next ordinary save writes the current format through the unchanged atomic storage path with the next revision. Before that first replacement the session always copies the unchanged older ciphertext, after checking that it still matches the authenticated file, to `<vault file>.schema-v<old version>-r<revision>.keyrook.bak` in the vault's folder (owner-only permissions, verified by reading back; a random suffix is added if a different file already has that name, and an identical copy is reused). The name is at most 255 UTF-8 bytes, the common file-name limit: a longer vault file name is cut at a character boundary and marked with `~` and the first eight hexadecimal digits of the SHA-256 of the full name, so the copy stays recognisable, is found again by a later save and differs between long names with a common beginning. The name of the copy is shown once after that save. This copy does not match the managed backup naming pattern, so rotation and the integrity check never touch it; it can be opened by the previous release or restored like any backup. If the copy cannot be written, the save fails with a specific error (`MigrationBackupException`) and the older file stays unchanged. If a session backup folder is configured, the existing pre-commit backup additionally copies the same older ciphertext into that folder, exactly as before every other save; that copy is subject to rotation.
